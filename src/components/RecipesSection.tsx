...
export default function RecipesSection() {
  interface Recipes {
    id: number;
    title: string;
    ingredients: string[]; // Update the type to be an array of strings
    description: string;
    steps: string[]; // Update the type to be an array of strings
    category: string;
    isFeatured: boolean;
    image: string;
  }
  const featuredRecipes = recipeData.recipes.filter(
    (recipes: Recipes) => recipes.isFeatured
  );
  return (
    <>
      <div className="h-auto w-full dark:bg-black bg-white  dark:bg-dot-white/[0.2] bg-dot-black/[0.2] relative flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
          {featuredRecipes.map((recipe: Recipes) => (
